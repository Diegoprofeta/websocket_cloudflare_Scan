...
\tWEBSOCKET SCANNER
\tBy : BETOVIP
\t  versão mais rápida (usando rosqueamento)
#'''+GR)

def update():
		statement = f"[{R}!{GR}] {O}Note ip intervalos atualizando semanalmente\n {G}Você deseja atualizar os intervalos de ip ?{G} "
...
def createlog():
	...
	with open('.firstusage.log',arg) as f:
			if len(f.read()) < 1:
				...
				input(f'{G}[#]Desligue o wifi ou mude para 0 balance sim \n então toque em Enter para continuar ')
				f.write(str(ranges))
			else:
				with open('.firstusage.log',arg) as f:
